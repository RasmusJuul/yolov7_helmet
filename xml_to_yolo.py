...
def xml_to_txt(input_path: str, output_path: str):
    ...
    for xml_file in glob.glob(input_path + '/*.xml'):      
        ...
        for member in root.findall("object"):
            ...
            if c in list(class_mapping.keys()):
                txt_list.append(class_mapping.get(c) + " " + " ".join([str(l) for l in bb]) + "\n")
            else:
                print(c)

        print(f"Building: {f_name}.txt")
        with open(output_path + "//" + f_name.split(".")[0] + ".txt", "w") as writer:
            for obj in txt_list:
                writer.write(obj)
    
def parser() -> None:
    parser = argparse.ArgumentParser(description="Converts .xml annotations into .txt files that conform to yolo format.")
